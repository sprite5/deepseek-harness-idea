...
| # | 验收项 | 状态 | 验证方式 | 备注 |
|---|---|---|---|---|
| 1 | 从磁盘安装构建出的 zip 到独立 IDEA 2025.1–2026.2（Windows）实例，无报错 | ⏳ 手工 | `gradle buildPlugin` → `deepseek-harness-idea-0.1.2.zip`（≈98MB，含运行时） | 本环境无法启动独立 IDE 会话；zip 结构已验证（plugin.jar + kotlin-stdlib + annotations）；2026.2 安装兼容已修复（v0.1.1，until-build 262.*） |
...
| 9 | 运行控制台选中日志右键"DSH 一键解释"，自动提交到 DSH 对话 | ⏳ 手工 | 真实 IDE 会话 + 运行中的 dsh（v0.1.11 修复后待复测） | 链路已就绪：`SendLogExplanationAction`（ConsoleView.PopupMenu）+ `DshToolWindowPanel.sendQuestion`（JCEF 填 composer + 派发回车 + JBCefJSQuery 结果回传）；降级路径（未运行/失败→剪贴板、阻塞→留在输入框）均有通知；**v0.1.11 修复根因**：注入脚本曾只查 `<textarea>`，而 dsh web ≥ 0.1.2 的 composer 已是 Lexical `contenteditable` → 必然 8s 后剪贴板兜底并提示 "Auto-send failed"（2026-09-11 用户截图报障）；现与"发送选中代码"共用 `[data-composer-input]` / `contenteditable` / `textarea` 三段选择器，回车派发到 composer 本体 |
...
6. 运行控制台"DSH 一键解释"的自动提交效果（v0.1.3-dev，FR-11；v0.1.11 已修根因——原脚本只查 `<textarea>`，需在真实 IDE 会话复测）。
